...
`kode/fibers` 在 v4.2.0 中继续打磨调度内核热路径（Channel 游标队列 + 内联、协程切换连续排空、spawn worker 借出内联、`bindFiber` 合入 `execute`、定时器惰性 id + 事件循环时钟量化共用桶），在 **PHP 8.3** + OPcache JIT 下与同类库（含原生协程引擎 Swoole / Swow）做真实横向压测。**Channel 与定时器双双登顶全场最快，反超原生 Swoole / Swow**：
...
| 协程切换（5 万次让出） | **7,537,689** | swow 19.9M / swoole 14.2M（原生） | PHP 用户态最快，领先 amphp/revolt **4.3x** | 19.5 KB |
