...
`kode/fibers` 在 v4.1.0 中针对调度内核做了系统性优化（就绪队列改为原生数组 + 游标、定时器二叉最小堆按到期时刻分桶、取消闭包零分配、修复协程结束后 `WeakMap` 内存泄漏），在 **PHP 8.3** 下与同类库做真实横向压测，协程级维度全面领先：

| 场景 | kode/fibers (ops/s) | 最接近同类 | 相对倍数 | 内存增量 |
| --- | ---: | ---: | ---: | ---: |
| 协程创建与完成（2 万） | **215,949** | amphp/amp 119,886 | **1.80x** | 0 B |
| 协程切换（5 万次让出） | **1,694,300** | amphp/amp 1,138,140 / revolt 1,133,082 | **1.49x** | 0 B |
| 定时器调度（2 万 0 延迟） | **397,889** | reactphp 249,999 / revolt 536,829 | 领先 reactphp 1.59x | 0 B |
| Channel 吞吐（5 万消息） | **1,686,848** | amphp/pipeline 1,116,432 | **1.51x** | 0 B |
| 并发聚合（1 万任务） | **180,681** | amphp/amp 132,935 | **1.36x** | 0 B |

> 数值为预热 2 轮 + 实测 5 轮取中位数（PHP 8.3.x，`hrtime` 计时）。`reactphp`/`revolt` 的「回调」维度为纯事件循环（无协程语义），仅作参考，不与协程级实现直接等价对比。完整方法论、环境与逐场景曲线见 [docs/benchmark.md](docs/benchmark.md)。
